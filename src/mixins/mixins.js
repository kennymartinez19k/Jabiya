...
export const Mixins = {
            ...
            this.setGps().then((res) => {
                if(res){
                    Geolocation.getCurrentPosition().then((myLocation) => {
                        ...
                        if(Math.abs(location.latitude - this.lastLocation.latitude) > 0.0001 || Math.abs(location.longitude - this.lastLocation.longitude) > 0.0001){
                            this.lastLocation = {...location}
                            services.gpsServices.updateLocation(this.user.id, location.latitude, location.longitude, this.load.bay_id._id )
                        }
                    })
                }
            })
    ...
    methods: {
         ...
         async driverArrival(val){
            ...
            this.load = val
            services.gpsServices.updateFirstLocation(this.user.id, location.latitude, location.longitude, this.load.bay_id._id )
         },
